<script>
  ...
  async function fetchDeliveryOrder(transfer_note_id) {
    console.log(`http://${$uri}:8888/orders/delivery/transfernote/${transfer_note_id}`);
  ...
  async function fetchDeliveryOrderDetails(delivery_order_id) {
    console.log(`http://${$uri}:8888/orders/delivery/detail/${delivery_order_id}`);
      ...
      console.log(delivery_order_details);
  }
              ...
              {#if delivery_orders.length > 0 }
                ...
                {#if showSuratJalan && idSuratJalan === delivery_order.delivery_order_id}
                  ...
                  <div class="flex items-center justify-start">
                    <button type="button" 
                    on:click={() => exportPDF(window.location.origin+`/print_delivery_order/${$uri}/${delivery_order.delivery_order_id}/${from.store_warehouse_id}/${to.store_warehouse_id}`, `DeliveryOrder_${delivery_order.delivery_order_id}_${formatDate(delivery_order.order_timestamp)}`, 190)}

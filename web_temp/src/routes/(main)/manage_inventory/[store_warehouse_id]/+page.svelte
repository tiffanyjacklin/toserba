            ...
            {#if showFilter}
              <div class="shadow-[0_2px_3px_rgba(0,0,0,0.3)] absolute right-0 z-10 mt-2 w-3/5 bg-gray-100 p-4 rounded-lg font-roboto">
                ...
                <div class="flex justify-between font-semibold mt-4">
                    ...
                    <button class="bg-[#f2b082] hover:bg-[#f7d4b2] transition-colors duration-200 ease-in-out text-[#364445] px-4 py-2 rounded" on:click={async() => {$loading = true; await fetchProduk(); toggleFilter(); clearVariable(); $loading = false;}}>Apply</button>
                </div>
              </div>
            {/if}
          </div>
          ...
          <div class="relative w-11/12 shadow-[0_2px_3px_rgba(0,0,0,0.3)] rounded-lg">
            ...
            {#if showFilter}
              <div class="shadow-[0_2px_3px_rgba(0,0,0,0.3)] absolute right-0 z-10 mt-2 w-3/5 bg-gray-100 p-4 rounded-lg font-roboto">
                ...
                <div class="flex justify-between font-semibold mt-4">
                    <button class="bg-gray-200 hover:bg-gray-300 transition-colors duration-200 ease-in-out px-4 py-2 rounded" on:click={() => {clearVariable()}}>Clear</button>
                    <button class="bg-[#f2b082] hover:bg-[#f7d4b2] transition-colors duration-200 ease-in-out text-[#364445] px-4 py-2 rounded" on:click={async() => {$loading = true; await fetchStockCardHistory(); toggleFilter(); clearVariable();  currentPage = 1; $loading = false;}}>Apply</button>
                </div>
              </div>
            {/if}
          </div>
    ...
    <TaskModal open={showModal} onClose={closeModal} color={"#3d4c52"}>
      ...
      <form class=" mt-4 p-4 md:p-5">
          <div class="grid gap-3 font-roboto font-semibold">
            ...
            <div class="">
              <button 
                on:click={() => viewPDF(window.location.origin+`/print_stock_card/${$uri}/${product.ProductDetails.product_detail_id}/${store_warehouse_id}`, `StockCard_${product.ProductDetails.product_detail_id}_${sw_name_print}_PrintedOn${getFormattedDateForPrint()}`, 190)}

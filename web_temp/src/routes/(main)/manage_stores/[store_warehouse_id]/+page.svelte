<script>
   ...
   function closeModal() {
    ...
    async function fetchSessionNotVerified() {
      ...
      if (data.status !== 200) {
        ...
        totalRows = filteredSessionsNot.length;
        totalPages = Math.ceil(totalRows/limit);
   ...
   async function verifySession(session_id, actual_closing_cash, deposit_money) {
    ...
    async function fetchDeliveryOrder() {
        const response = await fetch(`http://${$uri}:8888/orders/delivery/warehouse/to/${store_warehouse_id}/1/0/''/''/''/${limit}/${offset}`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json'
            }
        ...
        totalRows = data.total_rows;
        totalPages = Math.ceil(totalRows/limit);
        console.log("delivery_orders ", delivery_orders)
    }
...
<div class="mt-[90px] mx-8  mb-10 pb-10 p-3 flex flex-col items-center justify-center bg-white shadow-[inset_0_2px_3px_rgba(0,0,0,0.2)] rounded-lg">
    ...
    <span class="text-4xl font-bold font-roboto text-[#364445] my-10">Manage Stores</span>
      
    <div class="w-11/12 my-10">
        <div class="grid grid-cols-4 gap-x-2 gap-y-4">
            {#if tampilan == "session"}
                <button on:click={async() => {tampilan = "session"; tampilan = tampilan;}} class="bg-peach2 text-darkGray font-semibold text-xl w-56 py-2 rounded-3xl border-2 border-darkGray">Session Not Verified</button>
            {:else}
                <button on:click={async() => {tampilan = "session"; tampilan = tampilan; goToPage(1);}} class="bg-darkGray text-white font-semibold text-xl w-56 py-2 rounded-3xl border-2 border-darkGray">Session Not Verified</button>
            {/if}
            {#if tampilan == "session_all"}
                <button on:click={async() => {tampilan = "session_all"; tampilan = tampilan;}} class="bg-peach2 text-darkGray font-semibold text-xl w-56 py-2 rounded-3xl border-2 border-darkGray">All Session</button>
            {:else}
                <button on:click={async() => {tampilan = "session_all"; tampilan = tampilan; goToPage(1);}} class="bg-darkGray text-white font-semibold text-xl w-56 py-2 rounded-3xl border-2 border-darkGray">All Session</button>
            {/if}
            {#if tampilan == "transaction"}
                <button on:click={async() => {tampilan = "transaction"; tampilan = tampilan;}} class="bg-peach2 text-darkGray font-semibold text-xl w-56 py-2 rounded-3xl border-2 border-darkGray">Transaction</button>
            {:else}
                <button on:click={async() => {tampilan = "transaction"; tampilan = tampilan; goToPage(1);}} class="bg-darkGray text-white font-semibold text-xl w-56 py-2 rounded-3xl border-2 border-darkGray">Transaction</button>
            {/if}
            {#if tampilan == "verify_delivery"}
                <button on:click={async() => {tampilan = "verify_delivery"; tampilan = tampilan; }} class=" bg-peach2 text-darkGray font-semibold text-xl w-56 py-2 rounded-3xl border-2 border-darkGray">Verify Delivery</button>
            {:else}
                <button on:click={async() => {tampilan = "verify_delivery"; tampilan = tampilan; goToPage(1)}} class=" bg-darkGray text-white font-semibold text-xl w-56 py-2 rounded-3xl border-2 border-darkGray">Verify Delivery</button>
            {/if}
        </div>    
    </div>
            ...
            <div class="w-[96%] my-5 font-roboto">
                ...
                {#each delivery_orders as order}
                    ...
                    <div class="w-10/12 flex flex-col font-semibold text-lg">
                        {#await getStoreWarehouse(order.store_warehouse_from)}
                        <span class="my-1">Loading...</span>
                        {:then warehouse_name}
                        <span class="my-1">From warehouse {warehouse_name}</span>
                        {/await}
                        {#await getStoreWarehouse(order.store_warehouse_to)}
                        <span class="my-1">Loading...</span>
                        {:then store_name}
                        <span class="my-1">To Store {store_name}</span>
                        {/await}
                        <div class="flex items-center my-1">
                            <span class="mx-1">{order.quantity_total} Items, </span>
                        {#if order.status_verify == 0}
                    ...
                    <div class="w-2/12 flex justify-end items-center">
                        <button on:click={() => goto(`/manage_stores/${store_warehouse_id}/delivery_order/${order.delivery_order_id}`)} class="p-4 bg-darkGray rounded-xl"><svg width="60" height="60" viewBox="0 0 60 60" fill="none" xmlns="http://www.w3.org/2000/svg">

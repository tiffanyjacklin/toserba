<script>
    ...
    function navigateToStorePage(store_warehouse_id) {
        const url = `/manage_cashier/${store_warehouse_id}`;
        goto(url);
    }
                ...
                <button on:click={() => navigateToStorePage(store.store_warehouse_id)} class="my-4 p-4 border-2 border-black rounded-xl w-11/12 hover:bg-gray-200">
                    <div class="flex">
                        ...
                        <div class="w-10/12 flex flex-col items-start justify-center">
                            <span class="text-2xl font-semibold text-wrap my-1">{store.store_warehouse_name}</span>
                            <span class="text-2xl font-semibold text-wrap my-1 text-start">{store.store_warehouse_address}</span>
                        </div>
                    </div>
                </button>
            {/if}

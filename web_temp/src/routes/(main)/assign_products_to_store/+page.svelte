<script>
  ...
  async function fetchExpiredDate(product_idNya, quantityNya, unit_typeNya) {
      ...
      expired_dates = [...data.data];
      expired_dates = expired_dates.map(item => ({
  ...
  async function fetchStockOpname() {
      ...
      filteredStock = aggregateStockByProduct(all_stocks);
  ...
  function aggregateStockByProduct(stockList) {
    
    let stockMap = new Map();

    const transferredProductIds = new Set(products_to_transfer.map(product => product.product_detail_id));

    stockList.forEach(stock => {
        if (stock.expected_stock > 0 && !transferredProductIds.has(stock.product_detail_id)) {
            let existing = stockMap.get(stock.product_detail_id);
            if (existing) {
                existing.expected_stock += stock.expected_stock;
            } else {
                stockMap.set(stock.product_detail_id, {
                    product_detail_id: stock.product_detail_id,
                    product_name: stock.product_name,
                    unit_type: stock.unit_type,
                    expected_stock: stock.expected_stock
                });
            }
        }
    });
  ...
  function addProductToProductList(productIdnya, productNamenya) {
      ...
      // Reset inputs and filter stocks again
      product = [];
      productId = ''; 
      productName = '';
  ...
  // Function to filter products for sending (post-fetch)
  ...
  async function prepareProductsForSending(products) {
    ...
    for (const product of products) {
        // Fetch expired date, batch, and quantity for each product
        console.log(product);
        console.log(expired_dates);
        expired_dates = [];
        await fetchExpiredDate(product.product_detail_id, product.quantity, product.unit_type);
        console.log(expired_dates);
        // Check if expired_dates is an array and has items before proceeding
  ...
  // Main function to send the prepared products
  let products_to_send_fix = [];

  async function sendProducts() {
      // Prepare products with required information
      ...
      // products_to_send_fix = [];
      // return;
      if (products_to_send_fix.length === 0 ){
        console.error('HUWAAAAAAAAA');
        return;      
      }
      ...
      console.log("Inserted transfer note details", data);
      ...
      // Clear the transfer list and navigate to the stock transfer notes page
      products_to_transfer = [];
      products_to_send_fix = [];
      closeModal();
      storeId.set('');
  ...
  <div class="select-none font-roboto text-[#364445] mx-8 mt-[90px] mb-10 flex flex-col items-center justify-center bg-white shadow-[0_2px_3px_rgba(0,0,0,0.2)] rounded-lg">
    <div class="flex flex-col w-full pb-10 justify-center bg-white shadow-[inset_0_2px_3px_rgba(0,0,0,0.2)] rounded-lg">
              ...
              <tr class={i % 2 === 0 ? 'bg-yellow-100' : 'bg-white'}>                        
                ...
                <td class="px-1 py-2 text-center ">
                  <div class="flex justify-center">
                    <button on:click={() => toggleShowBatchExpiredDate(product.product_detail_id, product.quantity, product.unit_type)} type="button" 
                  ...
                  {#if showBatchExpiredDate && pd_id === product.product_detail_id}
                    <div class="">
      ...
      {#each stocks as stock}
        {stock.product_detail_id}, {stock.batch}, {stock.expected_stock} & 
      {/each}
      <br>
      All Stocks:
      ...
      {#each filteredStock as stock}
        {stock.product_detail_id}, {stock.expected_stock} & 
      {/each}
      <br>
      Product to Transfer:

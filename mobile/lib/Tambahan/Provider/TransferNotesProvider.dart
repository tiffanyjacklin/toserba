...
class TransferNotesProvider extends ChangeNotifier{
  ...
  UnmodifiableListView get listTransferNotes => UnmodifiableListView(_listTransferNotes);
  ...
  int getMaxPage() {
    ...
    return (_listTransferNotes.length.toDouble() / itemPerPage.toDouble()).ceil();
  }
  ...
  // fecth api
Future<void> fetchTransferNotes(int storeId,  {String search = '',
      String statusVerify = '',
      String statusSend = '',
      String startDate = '',
      String endDate = '',
  ...
  final link = 'http://leap.crossnet.co.id:8888/orders/transfer/notes/all/$search/$storeId//$statusVerify/$statusSend/$startDate/$endDate/$limit/0';
  print(link);
  isLoading = true;
  notifyListeners();
  ...
  if (response.statusCode == 200) {
    ...
    if (temp['status'] == 200) {
      print(temp);
      maxRow = transfer.StockTransferNotesWarehouse.fromJson(temp).totalRows!;
      if(isSearch ){
        paginatorController.currentPage = 0;
      }
      _listTransferNotes = transfer.StockTransferNotesWarehouse.fromJson(temp).data!;
    } else {
      _listTransferNotes = [];
    }
  } else {

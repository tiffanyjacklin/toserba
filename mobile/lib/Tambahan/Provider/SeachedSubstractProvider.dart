 ...
 /// An unmodifiable view of the items in the cart.
  ...
  List<DropdownMenuItem> makeListItemSuggestion(){
    ...
    for(var item in _suggestionItem){
      ...
      listItem.add(
        DropdownMenuItem(value : item, child: Text('${item.productName}, ${item.batch} - ${getOnlyDate(item.expiredDate!)}', style: TextStyle(fontSize: 16),))
      );
    }
    return listItem;
  }
  ...
  Future<List<opname.Data>> _fetchSuggestionName(String name) async {
    var batch = '';
    var unitType = '';
